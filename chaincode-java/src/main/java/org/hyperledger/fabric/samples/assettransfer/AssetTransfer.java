...
public final class AssetTransfer implements ContractInterface {
    ...
    public Asset DuplicateAsset(final Context ctx, final String assetID,
        ...
        /** First read the existing asset with given assetID by calling ReadaAsset .. it will handle the not existing asset exception itself**/
        Asset originalAsset = ReadAsset(ctx, assetID);

        /** clone the object to get duplicate object while original asset is not affected **/
        Asset duplicateAssest = (Asset)originalAsset.clone(); /** shalow copy **/

        ChaincodeStub stub = ctx.getStub();
        //Use Genson to convert the Asset into string, sort it alphabetically and serialize it into a json string
        String sortedJson = genson.serialize(duplicateAssest);
        stub.putStringState(assetID, sortedJson);
        /** Transfer the duplicate to new owner**/
        TransferAsset(ctx, assetID, newOwner);
        return duplicateAssest;
    }
    }
